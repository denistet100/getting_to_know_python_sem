...
cnt_bush = int(input('Введите количество кустов n:'))

berry_list = [random.randint(0, 20) for _ in range(cnt_bush)]

cnt_berry = []
for i in range(-1, cnt_bush-1):
    cnt_berry.append(berry_list[i-1]+berry_list[i]+berry_list[i+1])
    print(berry_list[i-1], berry_list[i], berry_list[i+1])
print(max(cnt_berry))
